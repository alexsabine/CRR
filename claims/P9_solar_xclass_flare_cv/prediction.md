# P9 — Pre-registered prediction (Session 7)
...
1. Source: NOAA SWPC X-class flare catalogue (X≥1.0 events).
2. Inclusion: full-disk X-class events 1976–latest update.
3. Statistic: CV = SD(inter-arrival) / mean(inter-arrival).
4. Single cohort (full catalogue).
...
median(CV) outside [0.08, 0.30] ⇒ Z₂ identification fails.
...
Solar flare inter-arrival is known Poissonian-ish; CV near 1.0 would mean Class C (noise-dominated) rather than Class A; a clear ~0.16 result would be non-trivial.
